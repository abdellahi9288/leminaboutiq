...
export default function SummaryCards({ totalIncome, totalExpenses, inventoryValue }: SummaryCardsProps) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 px-3 md:px-10 py-2">
      {cards.map((card, i) => (
        <div
          key={card.label}
          className={`rounded-2xl border p-2.5 md:p-5 text-center animate-fade-up stagger-${i + 1}`}
          style={{
            background: "var(--card)",
        ...
        >
          <div
            className="w-8 h-8 md:w-11 md:h-11 rounded-xl flex items-center justify-center mx-auto mb-1.5"
            style={{ background: "var(--green-wash)", color: "var(--green-brand)" }}
          >
            {card.icon}
          </div>
          <p className="text-[10px] md:text-[12px] font-tajawal font-bold mb-1" style={{ color: "var(--text-muted)" }}>
            {card.label}
          </p>
          <p className="text-[14px] md:text-[20px] font-bold font-tajawal leading-tight" style={{ color: "var(--green-deep)" }}>
            <AnimatedMRU value={card.value} delay={i * 100} />
          </p>
          <span className="text-[10px]" style={{ color: "var(--gold)" }}>MRU</span>
        </div>
      ))}
    </div>
  );
