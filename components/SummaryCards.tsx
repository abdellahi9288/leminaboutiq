...
export default function SummaryCards({ totalIncome, totalExpenses, inventoryValue }: SummaryCardsProps) {
  ...
  return (
    <div className="grid grid-cols-3 gap-4 px-5 md:px-10 py-3">
      {cards.map((card, i) => (
        <div
          key={card.label}
          className={`rounded-[20px] border p-5 text-center animate-fade-up stagger-${i + 1}`}
          style={{
            background: "var(--card)",
        ...
        >
          ...
          <p className="text-[12px] font-tajawal font-bold mb-2" style={{ color: "var(--text-muted)" }}>
            {card.label}
          </p>
          <p className="text-[17px] md:text-[20px] font-bold font-tajawal" style={{ color: "var(--green-deep)" }}>
            <AnimatedMRU value={card.value} delay={i * 100} />
            <span className="text-[12px] mr-1" style={{ color: "var(--gold)" }}> MRU</span>
          </p>
        </div>
      ))}
    </div>
