...
export default function SummaryCards({ totalIncome, totalExpenses, inventoryValue }: SummaryCardsProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5 px-3 md:px-10 py-2 shrink-0">
      {cards.map((card, i) => (
        <div
          key={card.label}
          className={`rounded-2xl border p-4 flex items-center gap-3 animate-fade-up stagger-${i + 1}`}
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-tajawal font-bold" style={{ color: "var(--text-muted)" }}>
              {card.label}
            </p>
            <p className="text-[20px] font-bold font-tajawal leading-tight" style={{ color: "var(--green-deep)" }}>
