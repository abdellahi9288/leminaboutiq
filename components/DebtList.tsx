...
export default function DebtList({ items, onPay, onDelete }: DebtListProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-6 animate-fade-up">
        ...
        <p className="text-[14px] mt-2 text-center max-w-[280px] leading-relaxed" style={{ color: "var(--text-muted)" }}>
          عند البيع بالدين ستظهر الديون هنا.
        </p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            <div className="p-4">
              ...
              {/* Product info */}
              <div className="rounded-xl p-2.5 mb-3" style={{ background: "var(--cream)" }}>
                <p className="text-[13px] font-tajawal text-center" style={{ color: "var(--text-muted)" }}>
                  {debt.description}
                </p>
              </div>
              ...
              <div className="rounded-full h-2 mb-2" style={{ background: "var(--sand-dark)" }}>
                <div
                  ...
                  style={{ width: `${paidPercent}%`, background: isPaid ? "#22c55e" : "var(--green-brand)" }}
                />
              </div>
                  ...
                  {payingId === debt._id ? (
                    <div className="flex-1">
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder="المبلغ"
                          className="flex-1 border rounded-xl px-3 py-2 text-[13px] nums focus:outline-none focus:border-[var(--green-brand)]"
                          style={{ background: "var(--cream)", borderColor: "var(--border)", color: "var(--text-body)" }}
                        />
                        <button
                          onClick={() => handlePay(debt)}
                          disabled={saving}
                          className="btn btn-success btn-sm font-tajawal font-bold px-3"
                        >
                          {saving ? "..." : "دفع"}
                        </button>
                        <button
                    ...
                    <>
                      <button
                        ...
                        className="btn btn-success btn-sm flex-1 font-tajawal font-bold"
                      >
                        تسجيل دفعة
                      </button>
                      <button
